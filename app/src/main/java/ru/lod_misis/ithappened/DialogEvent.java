...
//                    Controller.saveEvent(event);
    ...
    public void initDialogAddPastEvent(final Event event) {
        ...
        Parameter numberParameter = event.getParameters().where().equalTo("typeParameter", "Type: Number").findFirst();
        Parameter commentParameter = event.getParameters().where().equalTo("typeParameter", "Type: Comment").findFirst();
        Parameter markParameter = event.getParameters().where().equalTo("typeParameter", "Type: Mark").findFirst();


        if (numberParameter != null) {
            tv_numberParameter = (TextView) findViewById(R.id.tv_numberParameter);
            tv_numberParameter.setText(event.getParameters().where().equalTo("typeParameter", "Type: Number").findFirst().getNameParameter());
        }else {
            findViewById(R.id.rl_numberParameter).setVisibility(View.GONE);
        }

        if (commentParameter != null) {
            tv_commentParameter = (TextView) findViewById(R.id.tv_commentParameter);
            tv_commentParameter.setText(event.getParameters().where().equalTo("typeParameter", "Type: Comment").findFirst().getNameParameter());
        }else {
            findViewById(R.id.rl_commentParameter).setVisibility(View.GONE);
        }

        if (markParameter != null) {
            tv_ratingParameter = (TextView) findViewById(R.id.tv_ratingParameter);
            tv_ratingParameter.setText(event.getParameters().where().equalTo("typeParameter", "Type: Mark").findFirst().getNameParameter());
        }else {
            findViewById(R.id.rl_ratingParameter).setVisibility(View.GONE);
        }

        tv_ok.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String comment = et_commentParameter.getText().toString();
                int number = Integer.parseInt(et_numberParameter.getText().toString());
                int mark = rb_ratingParameter.getNumStars();
                Controller.savePastEvent(comment, number, mark);
                cancel();
            }
        });
